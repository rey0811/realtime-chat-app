...
const toggleFavorite = (e) => {
  ...
  if (dbdata.favorites && dbdata.favorites[messageId]) {
    ...
    firebase
      .database()
      .ref(`favorites/${message.uid}/${messageId}`)
      .remove();
...
const createMessageDiv = (messageId, message) => {
  ...
  // お気に入りONのとき、お気に入りリンクのアイコンを 塗りつぶしあり(fa-star) 
  if(dbdata.favorites && dbdata.favorites[messageId]) {
    let favoriteIcon = divTag.find(`.message__favorite-icon`);
...
const showRoom = (roomName) => {
  ...
  roomRef.on('child_removed', (childSnapshot) => {
...
const loadChatView = () => {
  ...
  favoritesRef.on('child_removed', (favSnapshot) => {
    ...
    let favoriteIcon = $(`#message-id-${messageId}`).find(`.message__favorite-icon`);
  ...
  favoritesRef.on('child_added', (favSnapshot) => {
    ...
    dbdata.favorites[messageId] = favorite.message;
...
// 削除ボタンがクリックされたらメッセージを削除する
$(document).on('click','.message__delete', function(e) {
  
  console.log(e.target);
  
  // 確認ダイアログを表示して、OKであれば削除する
	if(window.confirm('削除してもよろしいですか？')){
	  
	  // .message--sentの要素を取得
	  const elem = $(e.target).closest('.message--sent');
	  
	  // .message--sentから、messageId取得
	  // idには"message-id-"が含まれているので、空に置換する
	  let messageId = elem[0].getAttribute("id").replace("message-id-","")
    ...
    firebase
      .database()
      .ref(`messages/${currentRoomName}/${messageId}`)
      .remove();
